<template>
  <section class="bg-gray-100 px-5 pb-10 lg:py-20">
    <div class="flex flex-col items-center max-w-8xl xl:flex-col xl:mx-auto">
      <div class="max-w-2xl xl:w-1/3">
        <h1
          class="text-center font-bold text-test xl:text-center"
          style="font-size: 42px;"
        >
          Devpro Tools
        </h1>
        <div class="primary-line bg-test mx-auto xl:mx-auto"></div>
        <p class="text-lg text-center text-gray-800 mb-5 xl:text-center">
          Kuasai Bahasa Pemrograman dan Framework yang Diperlukan untuk Menjadi
          Web Developer Profesional
        </p>
      </div>

      <div
        class="container bg-white justify-center p-4 mx-auto rounded-xl md:shadow-xl lg:p-10 xl:w-2/3 xl:max-w-3xl"
      >
        <div
          class="max-w-4xl mx-auto flex flex-wrap items-center justify-center gap-y-8"
        >
          <div class="flex items-center justify-center" style="width: 25%;">
            <img
              src="~/static/images/tools/html.svg"
              alt="Html"
              class="block object-contain h-16"
            />
          </div>
          <div class="flex items-center justify-center" style="width: 25%;">
            <img
              src="~/static/images/tools/css.png"
              alt="Css"
              class="block object-contain h-14"
            />
          </div>
          <div class="flex items-center justify-center" style="width: 25%;">
            <img
              src="~/static/images/tools/tailwindcss.svg"
              alt="Tailwindcss"
              class="block object-contain h-18"
            />
          </div>
          <div class="flex items-center justify-center" style="width: 25%;">
            <img
              src="~/static/images/tools/js.svg"
              alt="Js"
              class="block object-contain h-14"
            />
          </div>
          <div class="flex items-center justify-center" style="width: 25%;">
            <img
              src="~/static/images/tools/vue.svg"
              alt="Vue"
              class="block object-contain h-12"
            />
          </div>
          <div class="flex items-center justify-center" style="width: 25%;">
            <img
              src="~/static/images/tools/react-2.svg"
              alt="React"
              class="block object-contain h-16"
            />
          </div>
          <div class="flex items-center justify-center" style="width: 25%;">
            <img
              src="~/static/images/tools/nodejs.svg"
              alt="Node"
              class="block object-contain h-14"
            />
          </div>
          <div class="flex items-center justify-center" style="width: 25%;">
            <img
              src="~/static/images/tools/go.png"
              alt="Go"
              class="block object-contain h-10"
            />
          </div>
          <div class="flex items-center justify-center" style="width: 25%;">
            <img
              src="~/static/images/tools/mongodb.svg"
              alt="Mongodb"
              class="block object-contain h-11"
            />
          </div>
          <div class="flex items-center justify-center" style="width: 25%;">
            <img
              src="~/static/images/tools/git.svg"
              alt="Git"
              class="block object-contain h-14"
            />
          </div>
          <div class="flex items-center justify-center" style="width: 25%;">
            <img
              src="~/static/images/tools/python.svg"
              alt="Python"
              class="block object-contain h-14"
            />
          </div>
          <div class="flex items-center justify-center" style="width: 25%;">
            <img
              src="~/static/images/tools/firebase.svg"
              alt="Firebase"
              class="block object-contain h-14"
            />
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script>
export default {};
</script>

<style></style>
